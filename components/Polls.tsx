...
const SurveyBuilderModal: React.FC<{ onClose: () => void; onSave: (survey: any) => void }> = ({ onClose, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!region.trim()) {
      showToast('A região alvo é obrigatória.', 'error');
      return;
    }

    // 2. Validação de Quantidade de Perguntas
    if (questions.length === 0) {
      showToast('A pesquisa deve ter pelo menos uma pergunta.', 'error');
      return;
    }

    // 3. Validação Individual de Perguntas
    const processedQuestions: Partial<Question>[] = [];

    for (const q of questions) {
      if (!q.text?.trim()) {
        showToast('Todas as perguntas devem ter um enunciado preenchido.', 'error');
        return;
      }

      const cleanQuestion = { ...q, text: q.text.trim() };

      // 4. Validação Específica para Múltipla Escolha
      if (q.type === QuestionType.MULTIPLE_CHOICE) {
        const validOptions = q.options?.filter(opt => opt.trim() !== '') || [];
        if (validOptions.length < 2) {
          showToast(`A pergunta "${q.text.substring(0, 20)}..." deve ter pelo menos 2 opções válidas.`, 'error');
          return;
        }
        cleanQuestion.options = validOptions;
      }

      processedQuestions.push(cleanQuestion);
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
          ...
          <button 
            type="submit"
            className="flex items-center justify-center gap-2 px-8 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 shadow-lg transition-all active:scale-[0.98]"
          >
